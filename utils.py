...
def get_scheduler(config, optimizer):
    ...
    if config.sch == 'StepLR':
        ...
        def lr_func(epoch): return epoch / config.warm_up_epochs if epoch <= config.warm_up_epochs else config.gamma**len(
            [m for m in config.milestones if m <= epoch])
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optimizer, lr_lambda=lr_func)
    elif config.sch == 'WP_CosineLR':
        def lr_func(epoch): return epoch / config.warm_up_epochs if epoch <= config.warm_up_epochs else 0.5 * (
...
class GT_BceDiceLoss(nn.Module):
    ...
    def forward(self, gt_pre, out, target):
        ...
        gt_loss = self.bcedice(gt_pre5, target) * 0.1 + self.bcedice(gt_pre4, target) * 0.2 + self.bcedice(
            gt_pre3, target) * 0.3 + self.bcedice(gt_pre2, target) * 0.4 + self.bcedice(gt_pre1, target) * 0.5
